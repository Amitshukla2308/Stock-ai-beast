...
if os.path.exists(DOTENV_PATH):
    ...
    print("✅ .env file loaded (BOM stripped).")
else:
...
def validate_token_file():
    ...
    try:
        ...
        if gen_time < session_start:
            print(f"⚠️ Token from previous session (Generated {gen_time.strftime('%Y-%m-%d %H:%M')}). Expired.")
            return None
...
def authenticate_fyers(headless_url=None, generate_url_only=False):
    ...
    if not headless_url:
        ...
        if existing_token:
            ...
            print(f"✅ Found valid Fyers access token (Expires: {expiry_str})") 
    ...
    if not headless_url:
        ...
        print("3. Copy the FULL URL of that redirected page and paste it below.")
        
        full_url = input("\n🔗 Paste Redirected URL here: ").strip()
    else:
        print(f"🤖 Headless Mode: Using provided URL/Code")
    ...
    if resp_json.get("s") == "ok":
        ...
        print(f"❌ Auth Failed: {resp_json.get('message')}")
